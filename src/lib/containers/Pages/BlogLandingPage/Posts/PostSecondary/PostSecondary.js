...
import style from './PostSecondary.scss'
...
class PostSecondary extends Component<Props> {
  ...
  render () {
    return (
      <div className={`col1 col2-tablet ${style.postSecondary}`}>
        {this.renderTitle()}
        {this.renderPostMeta()}
        <div className={'row'}>
          <div className={`col1 ${style.imageContainer}`}>
            {this.renderImage()}
          </div>
          <div className={`col1 ${style.contentContainer}`}>
            {this.renderContent()}
            {this.renderReadMore()}
          </div>
        </div>
